...
# Import centralized rate limiting from the main bot
try:
    from poly_data.rate_limiter import get_rate_limit_manager, EndpointType

    _use_centralized_rate_limiter = True
except ImportError:
    _use_centralized_rate_limiter = False


def rate_limit_book():
    """Apply rate limiting for order book API calls."""
    if _use_centralized_rate_limiter:
        get_rate_limit_manager().acquire_sync(EndpointType.BOOK)
    else:
        time.sleep(0.1)


def rate_limit_data_api():
    """Apply rate limiting for data API calls (prices-history)."""
    if _use_centralized_rate_limiter:
...
# This prevents socket exhaustion from creating new connections per request
_session = None
...
def generate_numbers(start, end, TICK_SIZE):
    ...
    while current < end:
        ...
        current += TICK_SIZE
        current = round(
            current, len(str(TICK_SIZE).split(".")[1])
...
def add_formula_params(curr_df, midpoint, v, daily_reward):
    ...
    curr_df["reward_per_100"] = (
        (curr_df["Q"] / curr_df["Q"].sum()) * daily_reward / 2 / curr_df["size"] * curr_df["100"]
    )
    return curr_df
...
def get_all_results(all_df, client, max_workers=5):
    all_results = []
    error_count = [0]  # Use list to allow mutation in nested function scope
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(process_with_progress, (idx, row)) for idx, row in all_df.iterrows()
        ]
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if result is not None:
                if isinstance(result, tuple) and result[0] == "error":
                    error_count[0] += 1
                    if error_count[0] <= 5:
                        print(f"Error fetching market: {result[1]}")
                    elif error_count[0] == 6:
                        print("Suppressing further market fetch errors...")
                else:
                    all_results.append(result)

            if len(all_results) % (max_workers * 2) == 0 and len(all_results) > 0:
                print(f"{len(all_results)} of {len(all_df)}")

    if error_count[0] > 0:
        print(f"Total market fetch errors: {error_count[0]}/{len(all_df)}")
...
def add_volatility_to_df(df, max_workers=2):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(process_volatility_with_progress, (idx, row))
            for idx, row in df.iterrows()
        ]

        for future in concurrent.futures.as_completed(futures):
            result = future.result()
            if result is not None and result != ("error", None):
                if isinstance(result, tuple) and result[0] == "error":
                    error_count += 1
                    if error_count <= 5:
                        print(f"Error fetching volatility: {result[1]}")
                    elif error_count == 6:
                        print("Suppressing further error messages...")
                else:
                    results.append(result)

            if len(results) % (max_workers * 2) == 0 and len(results) > 0:
                print(f"{len(results)} of {len(df)}")

    if error_count > 0:
        print(f"Total volatility errors: {error_count}/{len(df)}")
