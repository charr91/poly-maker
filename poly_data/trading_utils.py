...
def get_best_bid_ask_deets(market, name, size, deviation_threshold=0.05):
    ...
    if best_bid is not None and best_ask is not None:
        ...
        bid_sum_within_n_percent = sum(
            size
            for price, size in global_state.all_data[market]["bids"].items()
            if best_bid <= price <= mid_price * (1 + deviation_threshold)
        )
        ask_sum_within_n_percent = sum(
            size
            for price, size in global_state.all_data[market]["asks"].items()
    ...
    if name == "token2":
        ...
        if all(
            x is not None
            for x in [best_bid, best_ask, second_best_bid, second_best_ask, top_bid, top_ask]
        ):
...
def get_order_prices(
    best_bid, best_bid_size, top_bid, best_ask, best_ask_size, top_ask, avgPrice, row
):
